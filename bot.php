...
require_once 'redbeanphp/connect.php';
...
/**
 * Creating vk_api objet
 * Initialize vk variables
 */
...
$A_NOTIFICATION = $vk->buttonText('&#128276; Подписаться на новостную рассылку', 'green', ['command' => 'a_notif']);
$D_NOTIFICATION = $vk->buttonText('&#128277; Отписаться от новостной рассылки', 'red', ['command' => 'd_notif']);
...
if ($type == 'message_new') {
    ...
    if ($payload == 'news') {
        if (!$userdata['notification']) {
            $vk->sendButton($id, "Новости на 1&#8419; канале", [[$A_NOTIFICATION], [$B_BACK]]);
        } else {
            $vk->sendButton($id, "Новости на 1&#8419; канале", [[$D_NOTIFICATION], [$B_BACK]]);
        }
    }
    ...
    if ($payload == 'a_notif') {
        ...
        R::store($users);
        $vk->sendButton($id, "&#9989; Вы успешно подписались на рассылку!", [[$D_NOTIFICATION], [$B_BACK]]);
    }
    ...
    if ($payload == 'd_notif') {
        ...
        $users->notification = false;
        R::store($users);
        $vk->sendButton($id, "&#10071; Вы больше не будете получать новости. И вкусяншки тоже!", [[$A_NOTIFICATION], [$B_BACK]]);
    }
